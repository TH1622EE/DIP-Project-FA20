% Course:      EE 566 - Digital Image Processing
 ...
 % specific to the image evaluated in blocks with the intent of
...
tic
RGB = im2double(imread('image3.jpg')); % Read image to memory and convert to double
...
%compBlockV = blockproc(V,[8 8],compFunV); % Block processed compressed V channel matrix
...
funV = @(block_struct) mean2(block_struct.data).*ones(8,8); %Function handle expanded blockV
blockV = blockproc(V,[8 8],funV); % Expanded block processed V channel matrix
...
N = (r/8)*(c/8);  % The total number of blocks created by blockproc() function
...
%compElocal = blockproc(RGBgray,[8 8],compFunE); % Compressed matrix created using entropy function handle
...
funE = @(block_struct) entropy(block_struct.data).*ones(8,8); % Function handle for expanded Entropy block classification
Elocal = blockproc(RGBgray,[8 8],funE); % Expanded matrix created using entropy function handle
...
%compClocal1 = blockproc(rgbGraySquared,[8 8],compfunC1); % Creation of matrix using funC1 function handle
...
%compClocal2 = blockproc(RGBgray,[8 8],compfunC2); % Creation of matrix using funC2 function handle
...
funC1 = @(block_struct) mean2(block_struct.data).*ones(8,8); % Function handle to determine the mean of each block in rgbGraySquared
Clocal1 = blockproc(rgbGraySquared,[8 8],funC1); % Creation of matrix using funC1 function handle
funC2 = @(block_struct) abs(mean2(block_struct.data).*ones(8,8)).^2; % Function handle to determine absolute value of the mean value of each block of RGBgray squared
Clocal2 = blockproc(RGBgray,[8 8],funC2); % Creation of matrix using funC2 function handle
...
funDark = @(block_struct) darkenImage(block_struct.data).*ones(8,8); 
Vdark = blockproc(V,[8 8],funDark);

%funBright = @(block_struct) brightenImage(block_struct.data).*ones(8,8); 
%Vbrighten = blockproc(V,[8 8],funBright);
